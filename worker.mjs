...
export default {async fetch(request,env){
 ...
 if(path==='/api/news'&&request.method==='GET'){
  const {results}=await env.DB.prepare('SELECT id,category,title,body,date FROM posts ORDER BY id DESC').all();return json(200,results);
 }
 if(path.startsWith('/api/')){
  ...
  if(request.headers.get('Origin')&&request.headers.get('Origin')!==url.origin)return json(403,{error:'허용되지 않은 요청입니다.'});
  ...
  if(path==='/api/login')return json(200,{ok:true});
